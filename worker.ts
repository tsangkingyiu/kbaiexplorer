  ...
  async fetch(request: Request, env: Env, ctx: any): Promise<Response> {
    ...
    if (pathname === "/api/fetch-models" || pathname.endsWith("/api/fetch-models")) {
      try {
        ...
        const isAnthropic = targetUrl.toLowerCase().includes("anthropic.com");
    ...
    if (pathname === "/api/test-model" || pathname.endsWith("/api/test-model")) {
      try {
        ...
        const isAnthropic = targetUrl.toLowerCase().includes("anthropic.com") || cleanEndpoint.includes("messages");
        if (apiKey && typeof apiKey === "string" && apiKey.trim()) {
          const key = apiKey.trim();
          headers["Authorization"] = `Bearer ${key}`;
          if (isAnthropic) {
